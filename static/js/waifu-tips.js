...
const live2d_settings = {
    ...
    'homePageUrl': 'https://nanna7077.github.io/',  // 主页地址，可选 'auto'(自动), '{URL 网址}'
    'aboutPageUrl': 'https://nanna7077.github.io/', // 关于页地址, '{URL 网址}'
    'screenshotCaptureName': '3dhehe.png',   // 看板娘截图文件名，例如 'live2d.png'
}
// 模型列表
const live2d_models = [
    {
...
function initModel() {
    ...
    if (live2d_settings.tipsMessage)
        window.fetch(live2d_settings.tipsMessage)
            .then(res => res.json())
            .then(resjson => {window.waifu_tips = resjson; loadTipsMessage()});
...
function loadTipsMessage() {
    const result = window.waifu_tips;
